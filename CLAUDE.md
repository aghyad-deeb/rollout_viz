...
- **Conversation minimap**: `RightPanel/Minimap.tsx` + `utils/minimapScale.ts` — a bare 8px strip (right: 10) overlaying ChatView's `relative` wrapper as a SIBLING of the scroll container (the Cmd+C `:scope > div` mapping forbids nesting). Log-scaled role-colored blocks (click → scroll to message, hover tooltip), RAF-throttled viewport indicator, ResizeObserver remeasure on collapse/resize, tick overlays for deep-link/quote/local/global search in MessageCard's highlight-priority order. Hidden in Presentation Mode and when the transcript fits on screen. The scroller reserves `pr-7` so cards never run under the rail.
- **Transcript readability conventions** (from the 2026-08 three-lens audit; keep these when touching MessageCard/ChatView): role-aware rhythm in ChatView replaces uniform gaps (tool results `mt-1.5` hug their call, assistant turns `mt-6`, else `mt-4`); Inter + IBM Plex Mono are wired via `@theme` `--font-sans`/`--font-mono` in index.css; tool/file message bodies render `font-mono text-xs`; prose is capped `max-w-[90ch] leading-6`, reasoning runs one step down (`text-[13px] leading-[1.6]`); role headers are 12px uppercase running heads and collapsed cards show a one-line content excerpt; the long-card clamp is role-dependent (`max-h-40` system/file, `max-h-60` tool) with a hidden-line count on the reveal pill and a bottom Collapse pill; grade-quote marks are violet (user ephemeral highlights stay fuchsia) and search marks carry `dark:` variants; the `message.reasoning` field is honored by `normalizeAssistantMessage` (its MessageCard memo deps include it).
