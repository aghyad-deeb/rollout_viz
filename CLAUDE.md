...
- **Paper figure style**: capture setting `captureStyle: 'screen' | 'paper'` (localStorage `rollout_viz_capture_style`, paper width in its own `rollout_viz_capture_paper_width`, default screen) — Presentation-Mode exports styled for research papers per `environments/eval_envs/paper/figure_style/PLOTTING_STYLE.md`: Column 3.25in / Full 6.75in at 300 DPI design space (975/2025px), body typeset to 9.00pt (mono 8.36pt, micro-labels 7.59pt — ALL px-`!important` rules in `.capture-paper` that touch scaleCard-owned properties MUST be `calc(Npx * var(--paper-scale))`, enforced by the cascade-guard test in `captureStyles.test.ts`), ink-on-white only, ephemeral highlights flatten to bold, elisions to `[…]`, TASK card normalized to base size, PDF download by default (MediaBox in true points, FlateDecode lossless). Styling lives ONLY under `.capture-paper` on capture clones (isolation repeatedly proven by sha256-identical screen exports across paper round trips — keep it that way). Fonts are self-hosted (`public/fonts/` + `captureFonts.ts` base64-embeds them into snapshots — CDN fonts cannot embed and silently fall back to DejaVu); the vendored Inter is a VARIABLE face, so authored weights must stay on the 400/500/600/700 grid (guard test) or rendering shifts vs the old CDN static faces. Preview-modal downloads derive label/format/page-geometry from a settings SNAPSHOT taken with the blob, never live state.
