            ...
            {displayedFiles.length > 0 && (
              <div className="flex items-center gap-1">
                <button
                  ...
                  className={`text-xs px-2 py-1 rounded flex items-center gap-1 ${isDarkMode ? 'text-blue-400 hover:text-blue-300 hover:bg-blue-900/30' : 'text-blue-600 hover:text-blue-700 hover:bg-blue-50'}`}
                  title={`Select all ${displayedFiles.length} visible files`}
                >
                ...
                <button
                  ...
                  className={`text-xs px-2 py-1 rounded flex items-center gap-1 ${isDarkMode ? 'text-gray-400 hover:text-gray-200 hover:bg-gray-700' : 'text-gray-500 hover:text-gray-700 hover:bg-gray-100'}`}
                  title="Clear selection"
                >
                  <span className="material-symbols-outlined text-xs">check_box_outline_blank</span>
                  Clear
                </button>
              </div>
            )}
                ...
                <tr className={`text-left text-xs font-medium uppercase tracking-wider ${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>
                  ...
                  <th className="px-2 py-2 w-10">
                    {displayedFiles.length > 0 && (
                      <input
                        type="checkbox"
                        checked={displayedFiles.length > 0 && displayedFiles.every(f => selectedFiles.has(getFullPath(f)))}
                        onChange={(e) => {
                          if (e.target.checked) {
                            handleSelectAll();
                          } else {
                        ...
                        title={`Select all ${displayedFiles.length} files`}
                      />
                    )}
                  </th>
                  <th 
